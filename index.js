...
${gg} ${exists ? " exists " : "*Obrigado por usar o bot SHIRO-BOT* 💕"}
...
const exists = await conn.isOnWhatsApp (gg)
console.log(exists);
conn.sendMessage(id ,`╔ ═〘 *♥ ️SHIRO-BOT♥ ️* 〙╗

*BAKAA!!* Os bots não podem entrar devem ser feitos manualmente / pergunte ao proprietário do bot

${gg} ${exists ? " exists " : "*[ SHIRO-BOT ]* 💕"}`, MessageType.text)
}
...
${gg} ${exists ? " exists " : "*[ SHIEO-BOT ]* 💕"}`, MessageType.text)
}
...
├≽ ️!kick (@da pessoa)
├≽️ !kick all (dono do Grupo)
├≽️ !demote @
├≽️ !promote @
├≽️ !close 
├≽️ !opengrup
╰─────────
${gg} ${exists ? " " : "*obrigado por usar o SHIRO-BOT* 💕"}
_#tetap jaga kesehatan_ ^_^`, MessageType.text)
}
...
╰───────── ${gg} ${exists ? " " : "*Obrigado por usar o SHIRO-BOT* 💕"}`, MessageType.text)
}
